...
@SpringBootTest
public class QuestionServiceAcceptanceTest {
    ...
    public void create() throws Exception {
        Question question = questionService.create(user, new Question("test", "test"));
    ...
    public void findById_ID에해당하는데이터가없는경우() throws Exception {
        Optional<Question> question = questionService.findById(100L);
        assertThat(question.isPresent()).isFalse();
    }

    @Test
    public void findByIdAndNotDeleted_삭제된Question() throws Exception {
        Optional<Question> question = questionService.findById(3L);
        assertThat(question.isPresent()).isFalse();
    }
    ...
    public void update() throws Exception {
        Question question = questionService.update(user, 1, new Question("update", "update content"));
        assertThat(question).isNotNull();
    ...
    public void update_권한이없는경우() throws Exception {
        ...
        questionService.update(user, 1, new Question("update", "update content"));
    }

    @Test(expected = UnAuthorizedException.class)
    public void update_파라미터가NULL인경우() throws Exception {
        questionService.update(null, 1, new Question("update", "update content"));

    }

    @Test(expected = QuestionNotFoundException.class)
    public void update_해당Question이존재하지않는경우() throws Exception {
        questionService.update(user, 10, new Question("update", "update content"));
    }

    @Test(expected = QuestionNotFoundException.class)
    public void update_해당Question삭제된경우() throws Exception {
        questionService.update(user, 3, new Question("update", "update content"));
    }

    @Test
    @DirtiesContext
    public void deleteQuestion() throws Exception {
        questionService.deleteQuestion(user, 1);
    ...
    public void deleteQuestion_파라미터가NULL인경우() throws Exception {
        questionService.deleteQuestion(null, 1);
    }

    @Test(expected = QuestionNotFoundException.class)
    public void deleteQuestion_이미존재하지않는경우() throws Exception {
        questionService.deleteQuestion(user, 10);
    }

    @Test
    public void checkAuthority() throws Exception {
        assertThat(questionService.isOwnerOfQuestion(user, 1)).isTrue();
        assertThat(questionService.isOwnerOfQuestion(user, 2)).isFalse();
    }

    @Test
    public void checkAuthority_파라미터가NULL인경우() throws Exception {
        assertThat(questionService.isOwnerOfQuestion(null, 1)).isFalse();
    }
}